...
# Load data
file = pd.read_csv("data_h.txt", delim_whitespace=True, header = None)
...
# Constraint 2: If a vehicle visits a pickup location, the assigned delivery location must be visited
con1 = {}
for i in P:
    for k in K:
        con1[i,k] = model.addConstr(quicksum(x[i, j, k] for j in N) - quicksum(x[j,DID[i], k] for j in N)== 0)
