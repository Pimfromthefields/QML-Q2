...
model = Model ('Split delivery + heteregeneous fleet VRP Problem')
...
num_vehicle = 25
c = [20,20,20,20,20,20,20,20,20,20,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100]
fc = [100,100,100,100,100,100,100,100,100,100,4000,4000,4000,4000,4000,4000,4000,4000,4000,4000,4000,4000,4000,4000,4000]
...
# ---- Objective Function ----
model.setObjective (quicksum(d[i,j] * x[i,j,k] for i in N for j in N for k in K) + quicksum(fc[k]*z1[k] for k in K))
...
con6 = {}
for k in K:
    con6[j] = model.addConstr(quicksum(Q[j]*z[j,k] for j in range(1,len(N))) <= c[k])
...
for j in range(1,len(N)):
    for k in K:
        con14[j,k] = model.addConstr(z[j,k]<=M/Q[j]*quicksum(x[i,j,k] for i in N))

con15={}
for k in K:
    con15[k] = model.addConstr(z1[k] == 1-x[0,0,k])

con16={}
for k in K:
    for i in range(1,len(N)):
        for j in N:
            con16[i,j,k] = model.addConstr(z1[k] >= x[i,j,k])
...
if model.status == GRB.Status.OPTIMAL: # If optimal solution is found
    ...
    print('Costs:') 
    print(sum(fc[k]*x[0,j,k].x for k in K for j in range(1,len(N))))
